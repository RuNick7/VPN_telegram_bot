...
class NodeService:
    ...
    async def get_node(self, node_id: str) -> Optional[Dict[str, Any]]:
        """Get node by ID."""
        try:
            response = await self.client.get(f"/api/v1/nodes/{node_id}")
            return response
        except Exception:
            return None
    ...
    async def create_node(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new node."""
        return await self.client.post("/api/v1/nodes", data=data)

    async def update_node(self, node_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Update node."""
        return await self.client.put(f"/api/v1/nodes/{node_id}", data=data)

    async def delete_node(self, node_id: str) -> None:
        """Delete node."""
        await self.client.delete(f"/api/v1/nodes/{node_id}")
