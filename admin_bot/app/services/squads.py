...
class SquadService:
    ...
    async def get_squad(self, squad_id: str) -> Optional[Dict[str, Any]]:
        """Get squad by ID."""
        try:
            response = await self.client.get(f"/v1/squads/{squad_id}")
            return response
        except Exception:
            return None
    ...
    async def create_squad(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new squad."""
        return await self.client.post("/v1/squads", data=data)

    async def update_squad(self, squad_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Update squad."""
        return await self.client.put(f"/v1/squads/{squad_id}", data=data)

    async def delete_squad(self, squad_id: str) -> None:
        """Delete squad."""
        await self.client.delete(f"/v1/squads/{squad_id}")
