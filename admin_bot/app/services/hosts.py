...
class HostService:
    ...
    async def get_host(self, host_id: str) -> Optional[Dict[str, Any]]:
        """Get host by ID."""
        try:
            response = await self.client.get(f"/v1/hosts/{host_id}")
            return response
        except Exception:
            return None
    ...
    async def create_host(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new host."""
        return await self.client.post("/v1/hosts", data=data)

    async def update_host(self, host_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Update host."""
        return await self.client.put(f"/v1/hosts/{host_id}", data=data)

    async def delete_host(self, host_id: str) -> None:
        """Delete host."""
        await self.client.delete(f"/v1/hosts/{host_id}")

    async def bulk_create_hosts(self, hosts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Bulk create hosts."""
        return await self.client.post("/v1/hosts/bulk", data={"hosts": hosts})
